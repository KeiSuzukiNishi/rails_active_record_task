class ExercisesController < ApplicationController
  ...
  def exercise2
    ...
    #   * left_outer_joinsを使うこと
    @shops = Shop.left_outer_joins(foods: :order_foods).distinct.where(foods: { order_foods: { id: nil } }) #foods: :order_foodsの意味。foods: { order_foods: { id: nil } }の意味
  end
  ...
  def exercise4 
    ...
    @customer = Customer.joins(:orders).select("customers.*, { foods.price*COUNT(orders.*)} foods_price_sum").group("customers.id").order("orders_count DESC").first
  end
end
